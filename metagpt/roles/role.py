...
class RoleContext(BaseModel):
    ...
    def important_memory(self) -> list[Message]:
        """Get information corresponding to the watched actions"""
...
class Role:
    ...
    def _watch(self, actions: Iterable[Type[Action]]):
        """Monitor corresponding behavior"""
    ...
    def set_env(self, env: 'Environment'):
        """Set the environment where the role works. The role can talk to the environment and also receive messages from it by observing."""
        self._rc.env = env
    ...
    async def _think(self) -> None:
        ...
        if len(self._actions) == 1:
            # If there is only one action, that is the only option
            self._set_state(0)
            return
    ...
    def _publish_message(self, msg):
        """If the role belongs to the env, then the role's messages will be broadcast to the env"""
        if not self._rc.env:
            # If env does not exist, do not publish messages
            return
    ...
    async def handle(self, message: Message) -> Message:
        """Receive information and respond with action"""
    ...
    async def run(self, message=None):
        """Observe, and think and act based on the observation results"""
        if message:
