...
def columns_to_milvus_schema(columns: dict, primary_col_name: str = "", desc: str = ""):
    """Here we assume that the structure of columns is str: regular type"""
    fields = []
...
class MilvusStore(BaseStore):
    ...
    def search(self, query: list[list[float]], *args, **kwargs):
        ...
        Pay attention to the above description, is this logic serious? This should take a long time, right?
        """
        ...
        # FIXME: results contain id, but the actual value must be obtained by calling the query interface
        return results
